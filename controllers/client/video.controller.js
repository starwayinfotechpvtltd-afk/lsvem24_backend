...
exports.unlockPrivateVideo = async (req, res) => {
  try {
    ...
    let remainingCoins = unlockCost;
    let deductPurchased = 0;
    let deductEarned = 0;
    if ((user.purchasedCoin || 0) > 0) {
      deductPurchased = Math.min(user.purchasedCoin, remainingCoins);
      remainingCoins -= deductPurchased;
    }
    if (remainingCoins > 0) {
      deductEarned = Math.min(user.earnedCoin || 0, remainingCoins);
    }

    await Promise.all([
      Video.findOneAndUpdate({ _id: video._id }, { $set: { videoPrivacyType: 1 } }, { new: true }), // Change video privacy type to public
      User.findOneAndUpdate({ _id: video.userId }, { $inc: { coin: unlockCost, earnedCoin: unlockCost } }, { new: true }), // Increase video owner's coin count
      new History({
        userId: video.userId,
...
exports.getShorts = async (req, res) => {
  try {
    ...
    const userId = req.query.userId && req.query.userId !== "null" && req.query.userId !== "" && mongoose.Types.ObjectId.isValid(req.query.userId)
    ...
    const [shorts] = await Promise.all([
      Video.aggregate([
                ...
                $match: {
                  $expr: {
                    $and: [
                      ...
                      ...(userId ? [{ $eq: ["$userId", "$$user"] }] : [{ $eq: [true, false] }])
                    ],
                  },
                },
              },
            ...
            pipeline: [
              {
                $match: {
                  $expr: {
                    $and: [{ $eq: ["$videoId", "$$videoId"] }, { $eq: ["$likeOrDislike", "like"] }],
                  },
                },
              },
            ],
            ...
            pipeline: [
              {
                $match: {
                  $expr: {
                    $and: [{ $eq: ["$videoId", "$$videoId"] }, { $eq: ["$likeOrDislike", "dislike"] }],
                  },
                },
              },
            ],
        ...
        {
          $project: {
            like: { $max: [{ $ifNull: ["$like", 0] }, { $size: "$realLikes" }] },
            dislike: { $max: [{ $ifNull: ["$dislike", 0] }, { $size: "$realDislikes" }] },
            shareCount: 1,
            title: 1,
            videoType: 1,
            videoTime: 1,
            ...
            subscriptionCost: { $ifNull: ["$channel.subscriptionCost", 0] },
            videoUnlockCost: { $ifNull: ["$channel.videoUnlockCost", 0] },
            channelName: "$channel.fullName",
            channelImage: "$channel.image",
            totalComments: { $size: "$totalComments" },
            isSubscribed: {
...
exports.getAllLikeThis = async (req, res) => {
  try {
    ...
    let similarVideos = await Video.find({
      _id: { $ne: video._id },
    ...
    if (!similarVideos || similarVideos.length === 0) {
      similarVideos = await Video.find({
        _id: { $ne: video._id },
        videoType: { $eq: video.videoType },
        isActive: true,
      })
    ...
    const data = await Promise.all(
      similarVideos.map(async (similarVideo) => {
        const [totalViews, isSaved, isSubscribedChannel, channelUser] = await Promise.all([
          WatchHistory.countDocuments({ videoId: similarVideo?._id }),
          userId ? SaveToWatchLater.exists({ userId: userId, videoId: similarVideo._id }) : false,
          userId ? UserWiseSubscription.findOne({ userId: userId, channelId: similarVideo.channelId }) : null,
          User.findOne({ channelId: similarVideo.channelId }).select("fullName image channelId subscriptionCost videoUnlockCost channelType").lean(),
        ]);

        const now = new Date();
        const timeDiff = now - (similarVideo?.createdAt ? new Date(similarVideo.createdAt) : now);
        ...
        return {
          ...
          videoTime: similarVideo.videoTime || 0,
          videoType: similarVideo.videoType || 1,
          channelId: similarVideo.channelId || "",
          videoPrivacyType: similarVideo.videoPrivacyType || 1,
          user: {
            fullName: channelUser?.fullName || "Channel",
            image: channelUser?.image || "",
            channelId: channelUser?.channelId || similarVideo.channelId || "",
            subscriptionCost: channelUser?.subscriptionCost || 0,
            videoUnlockCost: channelUser?.videoUnlockCost || 0,
            channelType: channelUser?.channelType || 1,
          },
